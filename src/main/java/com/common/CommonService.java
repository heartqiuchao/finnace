...
public class CommonService {
	...
	public String listEntity(CommonListEntity listEntity) throws ParseException {
	    ...
	    if (searchValue!=null&&!(searchValue.equals(""))) {//顶层所有行模糊查询
	    		if((searchColumn!=null)&&!(searchColumn.equals(""))&&!(searchColumn.equals("ALL"))){
	    			...
	    			for(int i=0;i<searchColumns.length;i++){
	    				sArray.add(searchColumns[i]+" like '%" + searchValue + "%'");
	    			}
	    		}else{
	    			int i=0;
	    			...
	    			for(;i<cols.length;i++){
	    				sArray.add(cols[i]+" like '%" + searchValue + "%'");
	    			}
	    		}
	    }
	    ...
	    if(columnsearchvalue.size()>0){
	    	...
	    	for(int i=0;i<lengths-1;i++){
	    		...
	    		if(datesearchs.length==2){
	    			...
	    			columnsearchsql+=cols[searchcol.get(i)]+" between to_date('"+date1+"','YYYY-MM-DD') and to_date('"+date2+"','YYYY-MM-DD') and ";
	    		}else{
	    			columnsearchsql+=cols[searchcol.get(i)]+" like '%"+columnsearchvalue.get(i)+"%' and ";
	    		}
	    	}
    		...
    		if(datesearchs.length==2){
    			String date1=datesearchs[0].substring(0, 10);
    			String date2=datesearchs[1].substring(0, 10);
    			columnsearchsql+=cols[searchcol.get(lengths-1)]+" between to_date('"+date1+"','YYYY-MM-DD') and to_date('"+date2+"','YYYY-MM-DD') ";
    		}else{
    			columnsearchsql+=cols[searchcol.get(lengths-1)]+" like '%"+columnsearchvalue.get(lengths-1)+"%' ";
    		}
	    }//每个对应行模糊查询条件
	    ...
	    if(condition==null||condition.equals("")){
	    	...
	    	recordsTotal=(dao.query("FROM "+entity+" WHERE "+condition)).size();
        ...
        String searchSQL = "";
        String sql = "FROM " + entity +" as ";
        if (individualSearch != "") {
            searchSQL = " where " + individualSearch;
        }
        sql += searchSQL+" order by " + orderColumn + " " + orderDir;
